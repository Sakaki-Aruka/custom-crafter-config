...
- string_match の場合、`([0-9]+):(.+)` で示されるルールに従う必要があります。
  - 最初の `([0-9]+)` というパターンは、正規表現と代入された文字列の区切りをであるカンマのインデックスを示します。上記の例であれば、2 つめのカンマが文字列と正規表現の区切りであることを示しています。
  - コロンを挟んで右側の `(.+)` は正規表現と代入された文字列を記載する場であることを示します。
  - 指定されたインデックスのコロンの左側を正規表現、右側を実際の文字列として扱います。
- 型 `long` を用いる場合においても内部での演算では `double` が用いられるため、商や積が `.0` のような形で表されることがあります。
  - `{long:~~~}` のように、プレースホルダの先頭に `long:` と記述することで、計算結果が `long` へとキャストされ、小数点以下が切り捨てられます。  
...
- predicate: 
  ...
  - random: `formula` に記載された確率で `type`, `value` の操作を実行します。
  - tag: 素材となるアイテムのコンテナが持つ中身が `formula` を満たす場合にのみ操作を実行します。
...
- type:  
  ...
  - amount: 成果物の個数を変更する
  - set_storage_item: 成果物がバンドルである場合のみ、その中に配置するアイテムを指定する
  - item_define: `set_storage_item` で配置するアイテムの定義を読み込む (**成果物に変化なし**)
  - can_place_on: アドベンチャーモードのとき、成果物を配置することができるブロックを設定する
  - can_destroy: アドベンチャーモードのとき、成果物で破壊することができるブロックの種類を設定する。
  - repair_cost: 金床で修理する際に要求されるエンチャントレベルを設定する
  - enchant_book: エンチャントされた本の中身を設定する
  - firework: 花火に関する設定を行う
  - stew: 怪しげなシチューに関する設定を行う
...
- value: 
  - `type` に指定した値によって従うべき正規表現パターンが異なります。
    ...
    - amount: `limit=(true|false),amount=([0-9]{1,4})`
      ...
      - amount: 成果物の個数
    - set_storage_item: `name=([a-zA-Z_0-9]+),amount=(\\d+),times=(\\d+)`
      - name: `item_define` にて定義したアイテムの名前
      - amount: 1スロットに配置する量( 1 スロットにつき 127 個まで配置可能)
      - times: 上記アイテムを配置するスロット数
    - item_define: **引数を持たない**
    - can_place_on: `([A-Za-z_0-9,]+)`
      - アドベンチャーモードで設置可能なブロック ID (複数の場合はカンマ区切り)
    - can_destroy: `([A-Za-z_0-9,]+)`
      - アドベンチャーモードで破壊可能なブロック ID (複数の場合はカンマ区切り)
    - repair_cost: `\\d+`
      - リペアコスト
    - firework: 正規表現なし(使用可能な値は以下の通り)
      - 引数なし
        - `trail`: 爆発したのち、各要素が尾を引いて落ちるか否か
        - `flicker`: 爆発したのち、各要素が音を立てて点滅するか否か
      - 引数 1 個
        - shape: `ball`, `ball_large`, `burst`, `creeper`, `star` の中から 1 つ
          - ball: 小さい球形
          - ball_large: 大きい球形
          - burst: 縦に細長い爆発
          - creeper: クリーパーの顔
          - star: 星型
        - power: 花火の飛翔時間(花火の星には適用不可), 0 ~ 127 の範囲内で指定可(範囲外の場合はデフォルトの威力)。
         
      - その他
        - `color`: 花火の色を指定する。複数色指定可(各色の区切りは`/`)。`rgb=` を文頭に書いた場合は RGB で色を指定(各要素の区切りは `-`)、それ以外は色を名前で指定する
        - `fade`: `color` と指定方法は同じ

---

`container` では以下の設定を行うことができます。
  - プレースホルダを用いて素材が保持していたデータを使用する
  ...
  - `$PLAYER_([a-zA-Z_]+)$` のパターンに従う、プレイヤーに関する特殊な値を使用する
