...
`%` をそのまま表示したいときはバックスラッシュ `\` を利用して記号をエスケープします。  

中括弧によって構成される式の値を整数型、小数型にしたいときはそれぞれ `long:`, `double:` を式の開始直後に配置してください。  
また、任意の桁数へ丸め込みを行いたい場合には、`long->数字p` または `double->数字p:` を使用してください。  
`long->数字p` は、計算結果の指定した桁数目までの四捨五入を行います。  
`double->数字p` は、計算結果の小数点以下の値に指定した桁数目までの丸め込みを行います。  
